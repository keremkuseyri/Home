...
if st.session_state["authentication_status"]:
    ...
    with st.sidebar.expander("Sea Import/Export Yearly 📊"):
         st.page_link("pages/testdb2.py", label="Sea Import/Export Revenue List📊")
    ...
    dataframe5 = pd.read_excel(f'reports/air_import_employee_kpis/'+ filenames_selected + ".xlsx" )
    ...
    dataframe2 = pd.read_excel('reports/air_import_employee_kpis/'+ filenames_selected + ".xlsx")
    df2 = pd.DataFrame(dataframe2)

    dataframe4 = pd.read_excel('reports/air_export_employee_kpis/Kemal HAZNEDAR.xlsx')
    df4 = pd.DataFrame(dataframe4)

    Import_Mean=df2["all_count_mean"][0]
    Export_Mean=df4["all_count_mean"][0]
    ...
    fig.add_hline(y=Import_Forecast*contribution, line_dash='dash', line_color='orange', annotation_text=f'Prediction Target')
    fig.add_hline(y=Import_Forecast/3, line_dash='dash', line_color='purple', annotation_text=f'Count Per Employee Target')
